...
15. Sample wyłącznie w natywnej wysokości (decyzja właściciela): przy nowych recepturach żywe nagrania używaj dokładnie takie, jakie są w bibliotece — bez pola `pitch` (lub `pitch = 1.0`). Przepitchowany sample przestaje być rozpoznawalny i słuchacz słyszy hałas zamiast opisanego zdarzenia. Jeśli scena wymaga większego bestio-brzmienia, innego charakteru czy tempa, wybierz inny sample z biblioteki, nie przestrajaj go. Dozwolone przy samplu: `offset_sec` (wybór fragmentu), łagodny `hp_hz`, `gain`, `pan`. `scripts/render_jingle.py` odrzuca nowy render ze stemem o `pitch != 1`; historycznych artefaktów i ich receptur nie przepisuj.
...
3. **Remake:** kolejką steruje **najlepsza oceniona wersja fabuły**, nie najgorsza pojedyncza wersja. Szereguj fabuły rosnąco po ich najlepiej ocenionej wersji i remakuj fabułę o najniższym wyniku najlepszej wersji; nisko oceniony remake (np. v2 przy znacznie lepszym v1) nie podbija fabuły w kolejce ponad jej najlepszą wersję. Nowa wersja `vN` buduje na mocnych stronach najlepiej ocenionej wersji tej fabuły — oceny i komentarze wskazują, co poprawić, nie wymuszają kontynuacji po porażce. Fabuła, której najnowsza wersja czeka jeszcze na ocenę, nie wchodzi do kolejki (nie stackuj wersji). Kolejkę wypisze `python scripts/remake_queue.py --versions data/versions.json`. Jeśli nie ma ocenionej fabuły wymagającej poprawy, odnotuj brak zadania.
